namespace Sentinel.Tests.Integration.UserManagers
{
    ...
    public abstract class UserManagerTests
    {
        ...
        private IAsymmetricCryptoProvider asymmetricCryptoProvider;
        ...
        {
            var digest = new ApiKeyAuthenticationDigest(
                             username,
                             password,
                             "http://localhost",
            ...
            digest.SetSignature(signature);

            var user = await this.UserManager.AuthenticateUserWithApiKeyAsync(digest);
